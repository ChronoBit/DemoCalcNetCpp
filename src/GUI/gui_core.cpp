...
int GuiRender::loop() {
    ...
    constexpr int wWidth = 257;
    constexpr int wHeight = 380;
    
    const int sWidth = GetSystemMetrics(SM_CXSCREEN);
    const int sHeight = GetSystemMetrics(SM_CYSCREEN);
    
    int posX = (sWidth - wWidth) / 2;
    int posY = (sHeight - wHeight) / 2;
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = m_hWnd = ::CreateWindowW(wc.lpszClassName, L"Calculator",
        WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX,
        posX, posY, wWidth, wHeight, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    io.Fonts->AddFontFromMemoryTTF((void*)segoeui, sizeof segoeui, 28.0f, &cfg);
    ...
    while (!done) {
        ...
        const float clear_color_with_alpha[4] = {
            m_clearClr.x * m_clearClr.w, m_clearClr.y * m_clearClr.w, m_clearClr.z * m_clearClr.w, m_clearClr.w
        };
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            if (wParam == 0x38) {
                GuiRender::instance->onKeyDown(wParam, GetAsyncKeyState(VK_SHIFT));
            } else {
                GuiRender::instance->onKeyDown(wParam, GetKeyState(VK_SHIFT));
            }
